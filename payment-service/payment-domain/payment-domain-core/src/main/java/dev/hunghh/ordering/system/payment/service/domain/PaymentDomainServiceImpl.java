...
import dev.hunghh.ordering.system.payment.service.domain.valueobject.TransactionType;
...
public class PaymentDomainServiceImpl implements PaymentDomainService {
    ...
    public PaymentEvent validateAndInitiatePayment(Payment payment,
        ...
        if (failureMessages.isEmpty()) {
            ...
            return new PaymentFailedEvent(payment, ZonedDateTime.now(), failureMessages, paymentFailedEventDomainEventPublisher);
        }
    }

    @Override
    public PaymentEvent validateAndeCancelPayment(Payment payment,
                                                  CreditEntry creditEntry,
                                                  List<CreditHistory> creditHistories,
                                                  List<String> failureMessages,
    ...
    private void validateCreditEntry(Payment payment, CreditEntry creditEntry, List<String> failureMessages) {
        if (payment.getPrice().isGreaterThan(creditEntry.getTotalCreditAmount())) {
            ...
            failureMessages.add("Customer with id: " +
                    payment.getCustomerId().getValue() + " doesn't have enough credit for payment!");
        }
    }
